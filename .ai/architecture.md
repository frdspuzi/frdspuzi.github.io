...
**Change-detection before fetching (Unsplash only, 2026-07-27):** `fetch_unsplash.js` checks `total_photos` on the user profile and the Favourites collection first (2 cheap single-object API calls) and compares against the counts saved in `_data/unsplash_meta.json` from the last run — if both are unchanged, it skips the expensive paginated fetch entirely and exits. This exists because the script runs on a *fixed schedule* regardless of whether the profile actually changed, and fetching the *entire* photo history (see below) on every run, twice a day, whether or not anything's different, would burn meaningfully more of Unsplash's 50 req/hr free-tier budget than the old fixed-size (12/5/12 photo) fetches did. If another data source is ever changed to fetch "everything" instead of a bounded slice, consider the same pattern: a cheap count/metadata check first, a persisted "last known state" file, skip the expensive call when nothing's different.
