...
- **Cache-busted asset URLs:** every CSS/JS asset linked in `header.html`/`footer.html` (`styles.css`, and the 4 shared scripts loaded from `footer.html`) carries `?v={{ site.time | date: '%s' }}` — Jekyll's build timestamp as a query string, so the URL changes on every rebuild and a visitor's browser can never keep serving a stale cached copy after a CSS/JS change ships. Added 2026-08-02 after a real phone showed completely unstyled markup for a just-shipped feature while desktop DevTools' phone emulation (which reuses the desktop browser's already-current cache) looked correct — confirmed via `curl` that the live site itself was already serving the right HTML/CSS, so the only variable was the one browser's stale cached asset. **Any new asset added to `header.html`/`footer.html` needs the same `?v={{ site.time | date: '%s' }}` suffix from the start** — this class of bug is invisible from a dev environment (or DevTools emulation) and only surfaces on a real visitor's browser with a warm cache, so there's no way to "notice" it's missing without deliberately checking for it.
