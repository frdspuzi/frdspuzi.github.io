...
// Color classes below use Tailwind v4 arbitrary values referencing this site's own CSS custom
// properties (var(--surface), var(--border), etc.) instead of the component's original shadcn
// tokens (bg-card, text-muted-foreground, ...). Those tokens toggle via a `.dark` ancestor class
// (index.css's `@custom-variant dark (&:is(.dark *))`), which nothing on this site ever applies —
// this site's own dark mode toggles a `data-theme="dark"` attribute instead, a different, parallel
// convention. Left as shadcn tokens, this component would always render in shadcn's light palette
// regardless of the site's own theme toggle. Install-then-adapt: real structure/behavior/motion
// kept as-is, only the color source swapped to match this site's design system.
    ...
    >
      ...
      {item.description ? (
        <div
          ...
          className="mt-1 text-sm leading-6 text-[var(--fg-muted)]"
        >
          {item.description}
        </div>
      ) : null}
            ...
            <>
              <motion.span
                ...
                className={cn(
                  ...
                  highlighted ? "text-[var(--accent)]" : "text-[var(--border)]",
                )}
              />
            </>
          );
